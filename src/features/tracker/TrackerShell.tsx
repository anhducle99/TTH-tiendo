...
export function TrackerShell() {
  ...
  return <div className="app tracker-app">
    <aside className="side">
      ...
      </div>{(canReview || canManageUsers) && <><div className="navlabel nav-section">Quản trị</div><div className="sub root-sub">{canReview && <button className={page === 'approvals' ? 'on' : ''} onClick={() => navigate('approvals')}>Chờ duyệt</button>}{canManageUsers && <button className={page === 'users' ? 'on' : ''} onClick={() => navigate('users')}>Quản lý người dùng</button>}</div></>}</nav></div>
